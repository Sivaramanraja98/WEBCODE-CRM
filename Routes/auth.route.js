    ...
    try {
        ...
        if (existingUser) {
            ...
            if (isValidUser) {
                ...
                return res.status(200).send({ token: token, user: { _id, email, firstName,lastName,role } });
...
router.post('/submit-otp', async (req, res) => {
    try {
        ...
        payload.hashedPassword = await bcrypt.hash(payload.password, 10)
        delete payload.password;
        userModel.findOne({ otp: payload.otp }).then(result => {

            //update password
            userModel.updateOne({ email: result.email }, { hashedPassword: payload.hashedPassword })
                .then(result => {
